			...
			<c:forEach var="room" items="${room_list}">
				<div id="room_container" class="row">
					<div class="col" id="room_container_col" style="padding-left: 0px;">
						<li id="room_list"><span id="room_img"> <img
								src="${room.img}" loading="lazy">
								<div id="info_img">
								...
								<div id="room_info_time">
									<span> <i class="fas fa-clock" aria-hidden="true"></i>
										사용 가능 시간 &nbsp;
									</span> ${room.room_start}~${room.room_end}
								</div>
						</span>
							<div>
								<form id="detalform" action="searchDetail">
									...
									<input type="hidden" name="year" value="${year}"> <input
										...
										type="hidden" name="search_date" value="${search_date}">
									<input type="hidden" name="mapx" value="${room.mapx}">
									<input type="hidden" name="mapy" value="${room.mapy}">
									<input type="hidden" name="room_start"
										value="${room.room_start}"> <input type="hidden"
										name="room_end" value="${room.room_end}">
									<c:choose>
										<c:when test="${room.today_state eq true}">
											<input class="reservation_btn_on" type="button" value="예약하기"
												onclick="onver()">
										</c:when>
										<c:when test="${room.today_state eq false}">
							...
							</div> <!-- <td><a href="../Room/roomdetail.jsp">예약하기</button> --></li>
					</div>
				</div>
			</c:forEach>
		</ul>
